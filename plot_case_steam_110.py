"""
plot_case_steam_110.py — Visualise the steam-at-110 °C cascade screening.

Reads ``results/case_steam_110/case_steam_110.csv`` (produced by
``case_steam_110.py``) and writes five PNGs to the same folder.
...
Plots:
    ...
    3. eta_lorenz_grid.png  — η_Lorenz heatmap, V_ONLY hatched.
    4. Tdisch_grid.png      — max(T_disch_c1, T_disch_c2) per cell, 180 °C
                              reference contour.
    5. p_high_grid.png      — max(p_high_c1, p_high_c2) per cell, log scale,
...
# Two envelope modes:
#   default  → Ommen 2015 strict; reads case_steam_110.csv,        status col = status_4state, output filename suffix = ""
#   --modern → Project 68 modern; reads case_steam_110_enriched.csv,
#              status col = status_modern, output filename suffix = "_modern"
ENVELOPE_MODE = "ommen"  # overridden by CLI

# Module-level path globals — set per T_steam by main(); the helper
# functions below read them dynamically. Default values keep the
# 110 °C case importable as before.
CSV_PATH = os.path.join("results", "case_steam_110", "case_steam_110.csv")
CSV_PATH_MODERN = os.path.join("results", "case_steam_110",
                               "case_steam_110_enriched.csv")
OUT_DIR = os.path.join("results", "case_steam_110")
_T_STEAM_CURRENT = 110.0   # only used by the title strings
...
def load() -> pd.DataFrame:
    ...
    # Q_H consistency check (skip NOSOLVE rows)
    nn = df[df[_status_col()] != "NOSOLVE"]
    if not nn.empty:
        max_dev = (nn["Q_H_kW"] - 1000.0).abs().max()
        if max_dev > 0.1:
            print(f"  ! Q_H consistency: max deviation {max_dev:.3f} kW > 0.1 kW")
...
def plot_feasibility_grid(df: pd.DataFrame):
    ...
    fig.suptitle(
        f"Cascade feasibility — steam at 110 °C, Q_H = 1 MWth\n"
...
# ── Plot 2 & 3: COP and η_Lorenz heatmaps with V_ONLY hatch ────────────────
...
def plot_value_heatmap(df, value_col, fname, title, *,
    ...
    if last_im is not None:
        fig.tight_layout(rect=[0, 0.04, 0.93, 0.93])
...
def plot_Tdisch_grid(df: pd.DataFrame):
    ...
    fig.legend(handles=legend_handles, loc="lower center",
               ...
               bbox_to_anchor=(0.5, -0.02))
    fig.suptitle(
        "Compressor discharge temperature — max(T_disch_c1, T_disch_c2)\n"
        "Steam at 110 °C, Q_H = 1 MWth",
...
def plot_p_high_grid(df: pd.DataFrame):
    ...
    fig.legend(handles=legend_handles, loc="lower center",
               ...
               bbox_to_anchor=(0.5, -0.02))
    fig.suptitle(
        "High-side pressure — max(p_high_c1, p_high_c2)\n"
        "Steam at 110 °C, Q_H = 1 MWth",
        fontsize=12, fontweight="bold",
    )
...
def main(T_steam=None):
    ...
    plot_value_heatmap(
        ...
        title=f"COP — steam at {_T_STEAM_CURRENT:.0f} °C, Q_H = 1 MWth\n"
              "OK + V_ONLY shown; V_ONLY hatched",
        fmt="{:.2f}", cmap="viridis",
    )

    print("Plotting η_Lorenz heatmap ...")
    plot_value_heatmap(
        df, "eta_Lorenz", f"eta_lorenz_grid{_suffix()}.png",
        title="Second-law efficiency η_Lorenz = COP / COP_Lorenz\n"
              "Capacity-independent quality benchmark",
        fmt="{:.2f}", cmap="cividis", vmin=0.0, vmax=0.7,
    )
