"""
case_steam_110_economics.py — Exergoeconomic analysis for the OK designs of
the steam-at-110 °C case study (Project 68 modern envelope).

Pipeline:

  1. Load case_steam_110_enriched.csv produced by reclassify_modern.py.
  ...
  4. Run economics at the *native* simulation scale (m_steam = 1 kg/s,
     Q_H ≈ 2.23 MWth at 110 °C). The c_P [EUR/GJ] from this is the right
     intensive metric for ranking designs.
  5. Separately, compute PEC values for a 1 MWth-equivalent machine by
     applying the case-study scaling factor k = m_steam_target/1 to the
     sizing dict and re-evaluating the cost correlations. This 1 MWth PEC
     is for the Annex 58 / Project 68 €/kW band cross-check.
  6. Sensitivity sweep on electricity price (50–100 EUR/MWh, BASE 75).
  7. Write four CSVs to results/case_steam_110/economics/.

Run:
    python case_steam_110_economics.py
"""
...
from config import (
    ...
    E1_C_RANGE, Q_H_NOMINAL_KW, T_STEAM_CASE_110,
    lift_share_to_T34, p_water_for_T_steam,
)
from economics import (
    F_INSTALL, _CI_RATIO, COST_REF_YEAR, PUMP_REF_YEAR,
    pec_compressor, pec_motor, pec_pump, pec_plate_hx,
    run_economics,
)
...
# ── PEC at scaled sizing for 1 MWth comparison ──────────────────────────────

def _pec_at_scale(sim, k):
    """Compute total PEC and per-component breakdown at a sizing scaled
    by ``k`` (k = m_steam_target / m_steam_native = 1 for native sim).

    Returns a dict of component PEC values and total, all in reference-year
    EUR (with the same CI factor as run_economics applies)."""
    sz = sim["sizing"]
    f1 = sim["fluid_cycle1"]
    f2 = sim["fluid_cycle2"]
    ...
    # Apply scale: V_dot, W_comp, A_HX all scale linearly with m_steam.
    PEC_ref = {
        "COMP1":     (pec_compressor(sz["V_dot_comp1"] * k, f1, p_high_c1), COST_REF_YEAR),
        "COMP2":     (pec_compressor(sz["V_dot_comp2"] * k, f2, p_high_c2), COST_REF_YEAR),
        "SRC_PUMP":  (pec_pump(max(sz["W_src_pump"] * k, 1e-3)), PUMP_REF_YEAR),
        "SNK_PUMP":  (pec_pump(max(sz["W_snk_pump"] * k, 1e-3)), PUMP_REF_YEAR),
        "SRC_HX":    (pec_plate_hx(sz["A_src_hx"] * k, f1, p_high_c1), COST_REF_YEAR),
        "IHX":       (pec_plate_hx(sz["A_ihx"] * k, f1, p_high_c1), COST_REF_YEAR),
        "SNK_HX":    (pec_plate_hx(sz["A_snk_hx"] * k, f2, p_high_c2), COST_REF_YEAR),
        "MOT1":      (pec_motor(sz["W_comp1"] * k, f1, p_high_c1), COST_REF_YEAR),
        "MOT2":      (pec_motor(sz["W_comp2"] * k, f2, p_high_c2), COST_REF_YEAR),
    }
...
def main(T_steam=None):
    ...
    n = len(designs)
    print(f"Running economics on {n} OK (modern) designs at "
          f"LS ∈ {{0.30, 0.40, 0.50}}")
    ...
    print(f"  Q_H scaling for 1 MWth PEC: k_scale = "
          f"{Q_H_NOMINAL_KW / _dh_steam_kJ_per_kg(T_steam):.3f}")
    print()
    ...
    k_to_1MWth = Q_H_NOMINAL_KW / dh_steam   # m_steam_target [kg/s]
    ...
    for i, design in designs.iterrows():
        ...
        # Native-scale economics (Q_H ≈ 2.23 MWth at m_steam = 1 kg/s)
        Q_H_native_kW = dh_steam * 1.0  # 1 kg/s × Δh
        ...
        # 1-MWth PEC for Annex 58 / Project 68 €/kW comparison
        pec_1MW = _pec_at_scale(sim, k_to_1MWth)
        pec_total_1MW_EUR = pec_1MW["TOTAL"]
        pec_per_kW_1MW = pec_total_1MW_EUR / Q_H_NOMINAL_KW

        # Native-scale PEC (cross-check: this is ~2.23 MWth machine)
        pec_native = _pec_at_scale(sim, 1.0)
        pec_per_kW_native = pec_native["TOTAL"] / Q_H_native_kW
        ...
        base_rows.append({
            **common,
            "Q_H_native_kW": round(Q_H_native_kW, 1),
            "c_P [EUR/GJ]": round(eco["c_P"], 3),
            "Z_sum [EUR/h]": round(eco["Z_sum"], 3),
            "PEC_1MW_total [kEUR]": round(pec_total_1MW_EUR / 1000.0, 1),
            "PEC_1MW [EUR/kW]": round(pec_per_kW_1MW, 1),
            "PEC_native_total [kEUR]": round(pec_native["TOTAL"] / 1000.0, 1),
            "PEC_native [EUR/kW]": round(pec_per_kW_native, 1),
        ...
        # Component breakdown (1 MWth scaling)
        for comp, val in pec_1MW.items():
            if comp == "TOTAL":
                continue
            breakdown_rows.append({
                **common,
                "component": comp,
                "PEC_1MW [EUR]": round(val, 1),
                "PEC_1MW [EUR/kW]": round(val / Q_H_NOMINAL_KW, 2),
            })
        ...
        if (i + 1) % 10 == 0 or (i + 1) == n:
            print(f"{tag}  c_P={eco['c_P']:.2f} EUR/GJ  "
                  f"PEC_1MW/kW={pec_per_kW_1MW:.0f}  COP={common['COP']:.2f}")
    ...
    print(f"Our PEC_1MW range: {base_df['PEC_1MW [EUR/kW]'].min():.0f} to "
          f"{base_df['PEC_1MW [EUR/kW]'].max():.0f} EUR/kW "
          f"(median {base_df['PEC_1MW [EUR/kW]'].median():.0f})")
    in_band = ((base_df["PEC_1MW [EUR/kW]"] >= 400)
               & (base_df["PEC_1MW [EUR/kW]"] <= 700)).sum()
